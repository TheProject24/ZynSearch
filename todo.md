...
- [ ] Implement delta encoding (d-gaps): store document IDs as incremental differences, such as `[1000, 1004, 1012]` becoming `[1000, 4, 8]`, to shrink integer storage.
- [ ] Build a frame-of-reference (FOR) encoder: group document IDs into 128-byte blocks and compress them based on the maximum bits required for the largest value in each block.
- [ ] Integrate Roaring Bitmaps: dynamically swap out FOR compression for highly optimized bitwise bitmaps when handling terms with exceptionally dense document matches.
- [ ] Design a variable-byte (VByte) layout: use bit shifting to compress integers such as term frequencies and payload lengths so small values use 1 byte instead of 4 or 8.
